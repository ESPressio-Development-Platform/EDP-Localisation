...
def _parse_type_schema_json(
    ...
    for type_key, raw_type in types.items():
        type_id = parse_type_identifier(type_key, schema.type_identifier_bytes, path, f"$.types[{type_key!r}]")
...
def parse_schema_inventories(inputs: Iterable[Path]) -> SchemaUniverse | None:
    ...
    if not files:
        return None
    type_bytes: int | None = None
    field_bytes: int | None = None
    merged: dict[bytes, SchemaType] = {}

    for path in files:
        root = require_object(load_json(path), path, "$")
        allowed = {"schemaVersion", "typeIdentifierBytes", "fieldIdentifierBytes", "provenance", "types"}
        require_keys(root, {"schemaVersion", "typeIdentifierBytes", "fieldIdentifierBytes", "types"}, path, "$")
        ...
        tbytes = require_int(root["typeIdentifierBytes"], path, "$.typeIdentifierBytes")
        fbytes = require_int(root["fieldIdentifierBytes"], path, "$.fieldIdentifierBytes")
        if tbytes <= 0 or tbytes > 255:
            raise ToolError(f"{path}:$.typeIdentifierBytes: expected 1..255")
        if fbytes not in (1, 2, 4):
            raise ToolError(f"{path}:$.fieldIdentifierBytes: expected 1, 2, or 4")
        if type_bytes is None:
            type_bytes = tbytes
            field_bytes = fbytes
        elif type_bytes != tbytes or field_bytes != fbytes:
            raise ToolError(f"{path}: schema inventory identifier widths disagree with previous input")
        ...
        for type_key, raw_type in types.items():
            type_id = parse_type_identifier(type_key, tbytes, path, f"$.types[{type_key!r}]")
    ...
    assert type_bytes is not None and field_bytes is not None
    return SchemaUniverse(type_bytes, field_bytes, merged)
